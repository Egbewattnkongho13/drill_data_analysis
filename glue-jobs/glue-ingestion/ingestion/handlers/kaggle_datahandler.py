...
from core.handlers.data import DataSource
from core.sinks.base.sink import Sink
...
class KaggleDataHandler(DataSource):
    ...
    def __init__(self, urls: List[str], username: str, api_key: str):
        ...
        logger.info(f"Initialized KaggleDataHandler with {len(self.urls)} URLs.")
    ...
    def _setup_kaggle_credentials(self, username: str, api_key: str):
        ...
        if not username or not api_key:
            raise ValueError(
                "Kaggle credentials cannot be empty. Provide valid username and api_key."
            )
    ...
    def download(self, sink: Sink, destination: str) -> None:
        ...
        """
        logger.info(f"Downloading raw data from Kaggle URLs: {self.urls}")
